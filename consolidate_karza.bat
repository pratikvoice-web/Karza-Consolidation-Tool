...
echo ======================================================
echo   KARZA DEEP-CONSOLIDATION ENGINE - v2026.003
echo ======================================================
set "currentDir=%~dp0"
...
try {
    ...
    foreach ($f in $files) {
        ...
        $b3 = $ws.Range("B3").Text.Trim() # Legal Name
        $b4 = $ws.Range("B4").Text.Trim() # Trade Name
        $b5 = $ws.Range("B5").Text.Trim() # PAN
        $b6 = $ws.Range("B6").Text.Trim() # GSTIN
        $wb.Close($false)
        ...
        # Secure State & Suffix Extraction
    ...
    foreach ($fd in $fileDataList) {
        $k = $fd.PAN
        # If the 4th letter is 'P', it's a Proprietorship. Differentiate by Trade Name.
    ...
    # --- PHASE 2 & 3: EXTRACTION AND GENERATION ---
    foreach ($entry in $entityGroups.GetEnumerator()) {
        ...
        # Count states to check for multiple GSTINs in the same state
        $stateCounts = @{}
        ...
        foreach ($item in $entityItems) {
            ...
            # Apply Suffix dynamically if state has >1 GSTIN
            ...
            Write-Progress -Activity "Extracting Karza Data" -Status "Reading: $stHead" -PercentComplete (($fIdx / $entityItems.Count) * 100)
            Write-Host "      READING: $stHead" -ForegroundColor Gray

            $wb = $excel.Workbooks.Open($file.FullName)

            # DYNAMIC ROW FIX: Scan Related Parties without arbitrary 50 row limit
            foreach ($rpS in @("Related Party Sales - Monthly", "Related Party Purchases-Monthly")) {
                try { $wsRP = $wb.Sheets.Item($rpS)
                    $rpMaxRow = $wsRP.UsedRange.Rows.Count
                    for ($c=2; $c -le 200; $c+=8) { 
                        for ($r=4; $r -le $rpMaxRow; $r++) {
                            $rpP = $wsRP.Cells.Item($r, $c).Text.Trim()
                            if ($rpP.Length -eq 10) { $relatedPANs += $rpP }
                            if (-not $rpP -and $r -gt 10) { break } # Safe Break
                        } 
                    }
                } catch {}
            }
            ...
            # DYNAMIC ROW FIX: Scan Customer/Supplier Blocks until the end of the data
            foreach ($type in @("Customer", "Supplier")) {
                $ws = $wb.Sheets.Item("$type Wise - Monthly Data")
                $maxRow = $ws.UsedRange.Rows.Count
                
                # Column scan expanded to 400 (covers over 3 years of Karza data)
                for ($c = 1; $c -le 400; $c += 9) {
                    ...
                    for ($r = 4; $r -le $maxRow; $r++) {
                        ...
                        # Stop scanning this month's block if we hit "Total" or run out of valid records
            ...
            foreach ($mK in $fileMonths.Keys) { $d = $fileMonths[$mK]; $summaryData += [PSCustomObject]@{ Month=$mK; State=$stHead; Type="Customer"; GT=$d.GT; GI=$d.GI; IT=$d.IT_C; II=$d.II_C }; $summaryData += [PSCustomObject]@{ Month=$mK; State=$stHead; Type="Supplier"; GT=$d.GT; GI=$d.GI; IT=$d.IT_S; II=$d.II_S } }
            $wb.Close($false)
        }
        Write-Progress -Activity "Extracting Karza Data" -Completed
        ...
        $netCfgs = @(@{N="Tax. Value - Internal Sales"; G="GT"; I="IT"; T="Customer Taxable"; Tp="Customer"}, @{N="Inv. Value - Internal Sales"; G="GI"; I="II"; T="Customer Invoice"; Tp="Customer"}, @{N="Tax. Value - Internal Purchases"; G="GT"; I="IT"; T="Supplier Taxable"; Tp="Supplier"}, @{N="Inv. Value - Internal Purchases"; G="GI"; I="II"; T="Supplier Invoice"; Tp="Supplier"})
        
        foreach ($cfg in $netCfgs) {
            $step++; Write-Progress -Activity "Generating Workbook" -Status "Writing: $($cfg.N)" -PercentComplete (($step / $totalSteps) * 100)
            Write-Host "      WRITING: $($cfg.N)" -ForegroundColor Gray
            ...
            foreach ($mode in @("Gross", "Internal", "Net")) {
                ...
                $dr = $r + 2; foreach ($m in $months) {
                    ...
                    foreach ($s in $states) {
                        ...
                        $v = 0; if ($recs) { if ($mode -eq "Gross") { $v = ($recs | Measure-Object $($cfg.G) -Sum).Sum } elseif ($mode -eq "Internal") { $v = ($recs | Measure-Object $($cfg.I) -Sum).Sum } else { $v = ($recs | Measure-Object $($cfg.G) -Sum).Sum - ($recs | Measure-Object $($cfg.I) -Sum).Sum } }
        ...
        foreach ($mCfg in @(@{N="Detailed_Customer";D=$matrixCust | Where-Object { $_.Type -eq "Customer" }}, @{N="Detailed_Supplier";D=$matrixCust | Where-Object { $_.Type -eq "Supplier" }})) {
            $step++; Write-Progress -Activity "Generating Workbook" -Status "Matrixing: $($mCfg.N)" -PercentComplete (($step / $totalSteps) * 100)
            Write-Host "      MATRIXING: $($mCfg.N)" -ForegroundColor Gray
            ...
            foreach ($pg in ($mCfg.D | Group-Object PAN)) {
                ...
                foreach ($sg in ($pg.Group | Group-Object State)) {
                    $ws.Cells.Item($dr, 1) = "   >> " + $sg.Name; $cc = 3; foreach ($m in $months) { $v = ($sg.Group | Where-Object { $_.Month -eq $m } | Measure-Object T -Sum).Sum; if ($v -gt 0) { $ws.Cells.Item($dr, $cc) = $v }; $cc++ }
        ...
        $step++; Write-Progress -Activity "Generating Workbook" -Status "Finalizing..." -PercentComplete 100
        ...
        Write-Progress -Activity "Generating Workbook" -Completed
